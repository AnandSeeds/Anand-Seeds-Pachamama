...
void mensajeBienvenida() {
  char *mensaje1 = (char*) "    Bienvenido a SIE    ";
...
void enviarDatosSIM() {
  ...
  String cadena =
      "GET /index.php?data=0Ihzhj0geg_u16zk9AJNLlGl9F-9kE_bxeocU3n_RBOoDc-di1h93jvWz6chN9zBuF78S7NlmsMoYCF7NQ4-MeD5sqbkKWcF1onSaZz8EI-ABc1Ej1tNL-HMdr2YJS-N&id=1&ts="
          + String(ts) + "&ta=" + String(ta) + "&hs=" + String(hs)
          + "&hr=" + String(hr) + "&nuv=" + String(uvLevel) + "&iuv="
          + String(iuv) + "&lat=" + String(latit) + "&lon="
          + String(longi);
  ...
  if (key) {
    if (posicion == tamannoClave - 1) {
      ...
      if (iguales) {
        ...
        if (intentos <= 0) {
          while (true) {
            mostrarMensajeAutoscroll(
                (char*) "    Reinicie el sistema    ");
          }
        } else {
          intentos--;
          posicion = 0;
